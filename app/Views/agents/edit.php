        ...
        <form method="POST" action="/agents/<?= $agent['id'] ?>/edit">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="name" name="name" 
                               value="<?= htmlspecialchars($agent['name']) ?>" required
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="version" name="version" 
                               value="<?= htmlspecialchars($agent['version'] ?? '') ?>"
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="category" name="category">
                            ...
                            <option value="IDE" <?= ($agent['category'] === 'IDE') ? 'selected' : '' ?>>IDE/Editor</option>
                            <option value="Compiler" <?= ($agent['category'] === 'Compiler') ? 'selected' : '' ?>>Compiler</option>
                            <option value="Runtime" <?= ($agent['category'] === 'Runtime') ? 'selected' : '' ?>>Runtime</option>
                            <option value="Framework" <?= ($agent['category'] === 'Framework') ? 'selected' : '' ?>>Framework</option>
                            <option value="Database" <?= ($agent['category'] === 'Database') ? 'selected' : '' ?>>Database</option>
                            <option value="Container" <?= ($agent['category'] === 'Container') ? 'selected' : '' ?>>Container</option>
                            <option value="VCS" <?= ($agent['category'] === 'VCS') ? 'selected' : '' ?>>Version Control</option>
                            <option value="Testing" <?= ($agent['category'] === 'Testing') ? 'selected' : '' ?>>Testing Tool</option>
                            <option value="Build" <?= ($agent['category'] === 'Build') ? 'selected' : '' ?>>Build Tool</option>
                            <option value="Other" <?= ($agent['category'] === 'Other') ? 'selected' : '' ?>>Other</option>
                        </select>
                        ...
                        <select class="form-select" id="os_id" name="os_id">
                            ...
                            <?php if (!empty($os_list)): ?>
                                <?php foreach ($os_list as $os): ?>
                                    <option value="<?= $os['id'] ?>" <?= ($agent['os_id'] == $os['id']) ? 'selected' : '' ?>>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="install_path" name="install_path" 
                       value="<?= htmlspecialchars($agent['install_path'] ?? '') ?>"
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="notes" name="notes" rows="3"
                          placeholder="Additional notes about configuration, usage, or setup..."><?= htmlspecialchars($agent['notes'] ?? '') ?></textarea>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" 
                               value="<?= date('Y-m-d H:i:s', strtotime($agent['created_at'])) ?>" 
                               disabled>
...
document.getElementById('name').addEventListener('input', function() {
    ...
    if (!categorySelect.value || !pathInput.value) {
        const suggestions = {
